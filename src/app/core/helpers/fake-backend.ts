...
export let fakeBackendProvider = {
    ...
    useFactory: (backend: MockBackend, options: BaseRequestOptions) => {
        ...
        let statisticsjson : string = 
            '[{"jobId":1, "name":"OK", "value":"11111111111"},'+
            '{"jobId":1, "name":"OK", "value":"11111111111"},'+
            '{"jobId":1, "name":"OK", "value":"11111111111"},'+
            '{"jobId":1, "name":"OK", "value":"11111111111"},'+
            '{"jobId":1, "name":"OK", "value":"11111111111"},'+
            ...
            '{"jobId":1, "name":"OK", "value":"11111111111"},'+
            '{"jobId":1, "name":"OK", "value":"11111111111"}]';
        let statistics: any[] = JSON.parse(statisticsjson) || [];
        ...
        backend.connections.subscribe((connection: MockConnection) => {
            ...
            setTimeout(() => {
                ...
                // get statistics
                if (connection.request.url.endsWith('/api/statistics') && connection.request.method === RequestMethod.Get) {
                    // check for fake auth token in header and return users if valid, this security is implemented server side in a real application
                    if (connection.request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                        connection.mockRespond(new Response(new ResponseOptions({ status: 200, body: statistics })));
                    } else {
                        // return 401 not authorised if token is null or invalid
                        connection.mockRespond(new Response(new ResponseOptions({ status: 401 })));
                    }
